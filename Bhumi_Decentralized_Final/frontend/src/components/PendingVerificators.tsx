...
export default function PendingVerificators() {
  ...
  const [generatedKeys, setGeneratedKeys] = useState<{ address: string; privateKey: string; name: string } | null>(null);
        ...
        <div className="fixed inset-0 bg-moss-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-gray-900">Dompet Baru Berhasil Dibuat!</h3>
                <p className="text-xs text-moss-500 font-medium">Akun Pengguna: {generatedKeys.name}</p>
              </div>
            </div>

            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-bold text-moss-800 uppercase tracking-widest mb-1.5">Wallet Address (On-Chain)</label>
                <div className="flex gap-2">
                  <input readOnly value={generatedKeys.address} className="flex-1 px-3 py-2 bg-moss-50 border border-moss-100 rounded-lg font-mono text-xs text-moss-700 outline-none" />
                  <button onClick={() => { navigator.clipboard.writeText(generatedKeys.address); alert('Address berhasil disalin!'); }} className="px-3 py-2 bg-moss-100 hover:bg-moss-200 text-moss-700 font-bold text-xs rounded-lg transition-colors">Salin</button>
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-bold text-red-800 uppercase tracking-widest mb-1.5">Private Key (MetaMask)</label>
                <div className="flex gap-2">
                  <input type="text" readOnly value={generatedKeys.privateKey} className="flex-1 px-3 py-2 bg-red-50 border border-red-100 rounded-lg font-mono text-xs text-red-700 outline-none" />
                  <button onClick={() => { navigator.clipboard.writeText(generatedKeys.privateKey); alert('Private Key berhasil disalin!'); }} className="px-3 py-2 bg-red-100 hover:bg-red-200 text-red-700 font-bold text-xs rounded-lg transition-colors">Salin</button>
                </div>
              </div>
            </div>

            <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl mt-6">
              <div className="flex gap-2">
                ...
                <p className="text-[10px] text-orange-700 font-bold leading-relaxed">
                  PENTING: Harap salin dan berikan Private Key di atas kepada pengguna bersangkutan agar mereka dapat mengimpornya ke MetaMask mereka. Jangan menutup jendela ini sebelum Anda menyalin private key!
                </p>
              </div>
            </div>
